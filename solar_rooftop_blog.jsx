...
const CustomTooltipHD = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    return (
      <div style={{ background: "#0d1117", border: "1px solid #22d3ee", borderRadius: 8, padding: "8px 14px" }}>
        <p style={{ color: "#22d3ee", margin: 0, fontSize: 12 }}>{label}</p>
...
const CustomTooltipHW = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    return (
      <div style={{ background: "#0d1117", border: "1px solid #f59e0b", borderRadius: 8, padding: "8px 14px" }}>
...
const Tag = ({ children }) => (
  <span style={{
    ...
    border: "1px solid rgba(34,211,238,0.25)", fontFamily: "'Noto Sans KR', sans-serif"
  }}>{children}</span>
);
...
const SectionTitle = ({ children, accent }) => (
  <h2 style={{
    ...
    borderLeft: `4px solid ${accent || "#22d3ee"}`, paddingLeft: 14,
    lineHeight: 1.4
  }}>{children}</h2>
);

const InfoBox = ({ icon, title, desc, accent }) => (
  <div style={{
    background: `linear-gradient(135deg, rgba(${accent||"34,211,238"},0.08) 0%, rgba(15,23,42,0.6) 100%)`,
    border: `1px solid rgba(${accent||"34,211,238"},0.2)`,
    borderRadius: 12, padding: "18px 20px", marginBottom: 12
  }}>
    <div style={{ fontSize: 22, marginBottom: 6 }}>{icon}</div>
    <div style={{ color: "#e2e8f0", fontWeight: 700, fontSize: "0.95rem", fontFamily: "'Noto Sans KR', sans-serif" }}>{title}</div>
    <div style={{ color: "#94a3b8", fontSize: "0.85rem", marginTop: 4, fontFamily: "'Noto Sans KR', sans-serif", lineHeight: 1.6 }}>{desc}</div>
  </div>
);
...
export default function Blog() {
  ...
  return (
    <div style={{
      minHeight: "100vh", background: "linear-gradient(160deg, #040d1a 0%, #0a1628 50%, #060e1c 100%)",
      ...
      <div style={{
        ...
        <div style={{ position: "relative" }}>
          ...
          <p style={{ color: "#94a3b8", fontSize: "1rem", maxWidth: 560, margin: "0 auto 24px", lineHeight: 1.7 }}>
            RE100 인증이 필요한 수출 중소기업 사장님, 지금 당장 지붕을 '수익 자산'으로 바꾸세요.<br />
            초기 투자 0원 · 전기요금 20년 고정 · 지붕 무상 보수까지 — 태양광 지붕임대의 모든 것
          </p>
      ...
      <div style={{ maxWidth: 780, margin: "0 auto", padding: "0 20px 60px" }}>
        ...
        <div style={{ padding: "20px 0 4px", borderBottom: "1px solid rgba(255,255,255,0.06)", marginBottom: 32 }}>
          {["#태양광지붕임대", "#RE100중소기업", "#공장지붕태양광", "#전기요금절감", "#지붕임대수익",
            ...
            "#탄소중립경영", "#수출중소기업RE100", "#태양광투자", "#ESG경영", "#그린에너지"].map(t => (
              <Tag key={t}>{t}</Tag>
          ))}
        </div>
        ...
        <div style={{ background: "rgba(34,211,238,0.06)", border: "1px solid rgba(34,211,238,0.15)", borderRadius: 14, padding: "22px 24px", marginBottom: 28 }}>
          <p style={{ margin: 0, lineHeight: 1.9, fontSize: "0.95rem", color: "#cbd5e1" }}>
            "우리 공장 지붕, 그냥 놀리고 있어요." 많은 중소기업 대표님들이 하시는 말씀입니다. 
            수천 평의 공장·창고 지붕이 햇빛만 맞으며 묵묵히 노화되고 있습니다. 
            그런데 지금, 그 지붕이 <strong style={{ color: "#22d3ee" }}>20년짜리 '전기요금 동결 계약서'</strong>가 될 수 있습니다. 
            초기 투자 한 푼 없이, 태양광 발전사업자가 지붕을 빌려 쓰는 대신 여러분의 전기요금을 20년간 고정해주고, 
            지붕까지 무상으로 보수해줍니다. 이게 바로 지금 수출 중소기업들 사이에서 가장 빠르게 퍼지고 있는 
            <strong style={{ color: "#22d3ee" }}> '태양광 지붕임대 모델'</strong>입니다.
          </p>
        </div>
        ...
        <p style={{ lineHeight: 1.9, color: "#cbd5e1", fontSize: "0.93rem", marginBottom: 20 }}>
          수출 의존도가 높은 국내 중소기업 입장에서는 RE100이 더 이상 '선택'이 아닌 '생존 조건'이 되었습니다. 
          글로벌 바이어로부터 RE100 인증을 요구받는 사례가 급증하고 있으며, 
          이를 충족하지 못하면 납품 자체가 거절되는 상황까지 벌어지고 있습니다. 
          그런데 문제는 재생에너지를 직접 조달하는 방법이 복잡하고 비용이 많이 든다는 점입니다. 
          여기서 등장하는 해답이 바로 <strong style={{ color: "#22d3ee" }}>지붕 태양광 임대 모델</strong>입니다.
        </p>

        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 28 }}>
          <InfoBox icon="🌍" title="RE100 글로벌 가입 기업 수" desc="전 세계 400개 이상 기업 참여, 국내 삼성·SK·LG 등 대기업 포함 70여 곳 합류. 협력 중소기업으로 요구 범위 급속 확산 중" accent="34,211,238" />
          <InfoBox icon="📦" title="공급망 탄소 규제 강화" desc="EU 탄소국경조정제도(CBAM) 2026년 본격 시행. 탄소 배출량 높은 제품은 수출 시 추가 관세 부담. 재생에너지 전환이 곧 수출 경쟁력" accent="34,211,238" />
          <InfoBox icon="⚡" title="산업용 전기요금 상승세" desc="2023~2025년 산업용 전기요금 누적 35% 이상 인상. 전기 다소비 제조업 원가 압박 심화. 요금 고정화 니즈 폭발적 증가" accent="34,211,238" />
          <InfoBox icon="🏭" title="국내 공장 지붕 잠재면적" desc="전국 공장·물류창고 지붕 면적 추정 2억㎡ 이상. 현재 태양광 활용률 5% 미만. 잠자는 자산의 재발견 시대 도래" accent="34,211,238" />
        </div>

        <SectionTitle>2. 태양광 지붕임대 모델이란? — 핵심 구조 완전 해부</SectionTitle>
        <p style={{ lineHeight: 1.9, color: "#cbd5e1", fontSize: "0.93rem" }}>
          태양광 지붕임대(Third Party Ownership, TPO) 모델은 공장·창고 지붕 보유자와 태양광 발전사업자가 
          체결하는 장기 계약입니다. 구조는 단순합니다. 발전사업자가 모든 설치·운영 비용을 부담하는 대신, 
          지붕 보유자에게 아래의 혜택을 제공합니다.
        </p>
        ...
        <p style={{ lineHeight: 1.9, color: "#cbd5e1", fontSize: "0.93rem" }}>
          일반적인 지붕 임대는 '임대료'를 받는 구조입니다. 하지만 수출 중소기업 사장님들에게 임대료는 
        ...
        <p style={{ lineHeight: 1.9, color: "#cbd5e1", fontSize: "0.93rem", marginBottom: 24 }}>
          특히 2026년부터 EU 탄소국경조정제도(CBAM)가 철강·알루미늄·시멘트·화학·비료·전력 분야에 본격 적용되면서, 
          탄소 배출량 감축은 수출 원가와 직결됩니다. 재생에너지를 사용한다는 것은 
          단순한 '친환경 이미지'가 아니라, <strong style={{ color: "#22d3ee" }}>관세 비용을 줄이는 실질적 경제 무기</strong>가 됩니다.
        </p>
        ...
        {[
          ...
          ["🔧 유지보수 범위 명확화", "지붕 보수의 구체적 범위(방수층, 도장, 하자 보수)와 주기를 계약서에 명시. '무상 보수' 조항이 모호하면 분쟁 소지 발생 가능."],
        ].map(([title, body]) => (
        ...
        <div style={{ margin: "36px 0 0", padding: "28px 0 0", borderTop: "1px solid rgba(255,255,255,0.07)" }}>
          <h2 style={{
            ...
            borderLeft: "4px solid #f59e0b", paddingLeft: 14, lineHeight: 1.4
          }}>5. 에너지 종목 분석 — 투자자 관점에서 바라본 태양광 산업</h2>
          ...
          <div style={{ display: "flex", gap: 0, marginBottom: 24, background: "rgba(255,255,255,0.04)", borderRadius: 10, padding: 4 }}>
            {[["hd", "🔵 HD현대에너지솔루션 (322000)"], ["hw", "🟡 한화솔루션 (009830)"]].map(([key, label]) => (
          ...
          {activeTab === "hd" && (
            <div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 10, marginBottom: 20 }}>
                {[
                  ["현재 주가", "90,800원", "#22d3ee"],
                  ["시가총액", "8,960억원", "#22d3ee"],
                  ["EPS (TTM)", "2,743원", "#22d3ee"],
                  ["상장일", "2019년", "#94a3b8"],
                ].map(([label, val, color]) => (
              ...
              <div style={{ background: "rgba(13,17,23,0.8)", borderRadius: 14, padding: "20px 10px 10px", marginBottom: 20 }}>
                <div style={{ color: "#22d3ee", fontWeight: 700, fontSize: "0.9rem", marginBottom: 16, paddingLeft: 10 }}>📈 HD현대에너지솔루션 분기별 주가 추이 (2023~2026)</div>
                <ResponsiveContainer width="100%" height={200}>
                  <AreaChart data={hdData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                    <defs>
                    ...
                    <Tooltip content={<CustomTooltipHD />} />
                    <Area type="monotone" dataKey="price" stroke="#22d3ee" strokeWidth={2.5} fill="url(#hdGrad)" dot={{ fill: "#22d3ee", r: 3 }} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
              <div style={{ background: "rgba(34,211,238,0.05)", border: "1px solid rgba(34,211,238,0.12)", borderRadius: 12, padding: "18px 20px" }}>
                ...
                <p style={{ color: "#94a3b8", fontSize: "0.85rem", lineHeight: 1.8, margin: 0 }}>
                  ...
                  <br /><br />
                  2025년 3분기 누적 기준 전년 동기 대비 매출 9.3% 증가, 영업이익 흑자 전환에 성공했습니다. 
                  n-Type 고출력 제품 출시로 프리미엄 수요에 대응하고, 미국 RE100·24/7CFE 이행에 따른 상업용·유틸리티 수요 확대가 실적 개선을 이끌었습니다. 
                  2026년 1분기 현재 주가 <strong style={{ color: "#22d3ee" }}>90,800원</strong> 수준으로, 상장 이후 꾸준한 우상향 흐름을 이어가고 있습니다.
                  ESS 결합 태양광 발전시설 확산으로 분산에너지 활성화의 최대 수혜주로 꼽힙니다.
                </p>
              </div>
            </div>
          )}
          ...
          {activeTab === "hw" && (
            <div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 10, marginBottom: 20 }}>
                {[
                  ["2025 매출", "13.3조원", "#f59e0b"],
                  ["2025 영업손익", "-3,533억원", "#f87171"],
                  ["2026 목표주가", "30,000~47,000원", "#f59e0b"],
                  ["종목 코드", "009830", "#94a3b8"],
                ].map(([label, val, color]) => (
              ...
              <div style={{ background: "rgba(13,17,23,0.8)", borderRadius: 14, padding: "20px 10px 10px", marginBottom: 20 }}>
                <div style={{ color: "#f59e0b", fontWeight: 700, fontSize: "0.9rem", marginBottom: 16, paddingLeft: 10 }}>📈 한화솔루션 분기별 주가 추이 (2023~2026)</div>
                <ResponsiveContainer width="100%" height={200}>
                  <AreaChart data={hwData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                    <defs>
                    ...
                    <XAxis dataKey="date" tick={{ fill: "#64748b", fontSize: 11 }} axisLine={false} tickLine={false} />
                    <YAxis tick={{ fill: "#64748b", fontSize: 11 }} axisLine={false} tickLine={false} tickFormatter={v => `${(v/10000).toFixed(0)}만`} />
                    <Tooltip content={<CustomTooltipHW />} />
                    <Area type="monotone" dataKey="price" stroke="#f59e0b" strokeWidth={2.5} fill="url(#hwGrad)" dot={{ fill: "#f59e0b", r: 3 }} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
              <div style={{ background: "rgba(245,158,11,0.05)", border: "1px solid rgba(245,158,11,0.12)", borderRadius: 12, padding: "18px 20px" }}>
                ...
                <p style={{ color: "#94a3b8", fontSize: "0.85rem", lineHeight: 1.8, margin: 0 }}>
                  ...
                  자회사 한화큐셀을 통해 미국 조지아주에 북미 최대 태양광 통합 생산단지 '솔라 허브'를 구축하며 현지 자급 체계를 선점했습니다.
                  <br /><br />
                  2025년에는 연결기준 매출 <strong style={{ color: "#f59e0b" }}>13조 3,544억 원</strong>으로 역대 최대를 기록했으나, 
                  미국 통관 지연과 글로벌 공급과잉 여파로 영업손실 3,533억 원을 기록하며 2년 연속 적자를 이어갔습니다. 
                  ...
                  기관투자자들의 집중 매집이 진행 중이며, 증권사 목표주가는 30,000~47,000원 범위에 분포합니다.
                </p>
              </div>
            </div>
          )}
        ...
        {[
          ["STEP 1", "지붕 현황 파악", "지붕 면적(㎡), 건물 준공년도, 구조 형태(철골/PC/조적), 현재 전기요금 월 납부액을 정리합니다. 일반적으로 1,000㎡ 이상 지붕이면 사업자의 관심을 받을 수 있으며, 전기요금이 클수록 절감 효과도 큽니다.", "#22d3ee"],
          ["STEP 2", "복수 사업자 제안 비교", "한화솔루션, HD현대에너지솔루션, GS에너지, 민간 태양광 개발사 등 최소 3개 이상 사업자로부터 제안서를 받으세요. 단가, 계약 기간, 지붕 보수 범위, 해지 조건 등을 꼼꼼히 비교합니다.", "#22d3ee"],
          ["STEP 3", "법무·세무 검토 후 계약", "20년 장기 계약인 만큼 반드시 에너지 전문 법무사 또는 변호사의 계약서 검토를 거치세요. RE100 인증 절차, 전기요금 고정 단가의 세금계산서 처리 방식도 세무사와 확인하면 완벽합니다.", "#22d3ee"],
        ...
        <div style={{ background: "linear-gradient(135deg, rgba(34,211,238,0.08) 0%, rgba(10,22,40,0.8) 100%)", border: "1px solid rgba(34,211,238,0.2)", borderRadius: 16, padding: "24px 24px" }}>
          ...
          <p style={{ lineHeight: 1.9, color: "#94a3b8", fontSize: "0.88rem", margin: 0 }}>
            ...
            보여주듯, 태양광 산업은 지금 전환점에 있습니다. 공급과잉 우려를 딛고 미국 FEOC 규제, 
            중국산 보조금 폐지, AI 전력 수요 폭증이라는 구조적 수혜가 본격화되는 시점입니다. 
            그 흐름의 출발점이 바로 여러분의 공장 지붕일 수 있습니다.
          </p>
        </div>
        ...
        <div style={{ marginTop: 32, padding: "16px 20px", background: "rgba(255,255,255,0.02)", borderRadius: 10, border: "1px solid rgba(255,255,255,0.06)" }}>
          <p style={{ color: "#475569", fontSize: "0.75rem", lineHeight: 1.7, margin: 0 }}>
            ...
            태양광 지붕임대 계약 조건은 사업자마다 상이하므로 반드시 개별 확인이 필요합니다.
          </p>
        </div>
        ...
        <div style={{ marginTop: 28, paddingTop: 20, borderTop: "1px solid rgba(255,255,255,0.06)" }}>
          ...
          {["#태양광지붕임대", "#RE100", "#공장지붕태양광", "#전기요금절감20년", "#지붕임대수익",
            ...
            "#재생에너지투자", "#탄소중립", "#ESG경영", "#그린에너지", "#CBAM대응", "#에너지전환"].map(t => (
              <Tag key={t}>{t}</Tag>
          ))}
        </div>
      </div>
